  ...
  <div class="calendar" v-if="calendarInit">
    ...
    <div class="calendar__main">
      ...
      <div
        ...
        v-for="(item, index) in outerCalendarData[selectedMonth] ||
...
export default {
  ...
  mounted() {
    ...
    this.$nextTick(function () {
      this.blockHeight =
        document.querySelector(`.main__block-${this.calendarID}`).offsetWidth +
        'px'
    })
  },
  methods: {
    init() {
      const initDate =
        this.targetDate ||
        dayjs(this.targetTimestamp || Date.now()).format('YYYY/MM/DD')
    ...
    handleDayClick(item) {
      if (item.type === 'normal') {
        ...
        this.$emit('selectDate', {
          date: `${this.selectedYear}/${this.selectedMonth + 1}/${
            this.selectedDate
          }`
        })
      }
